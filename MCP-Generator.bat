...
echo Choose: [1.8, 1.12]
SET /p mainversion=#
if '%mainversion%' == '1.8' goto mainversion1.8 (
) else (
if '%mainversion%' == '1.12' goto mainversion1.12 (
) else (
goto syntaxerror
)
...
:mainversion1.12
cls
echo Good! You chose 1.12.
echo Now it's time to chose the subversion.
echo Choose: [1.12.0]
SET /p subversion=#
if '%subversion%' == '1.12.0' goto subversion1.12.0 (
) else (
goto syntaxerror
)
...
:subversion1.12.0
cls
echo Great! I will now install and setup your MCP 1.12.0. You can now lay back for a few minutes.
if exist MCP-1.12.0 (
goto folderalreadyexistserror
) else (
echo.
echo Downloading MCP-1.12.0...
echo.
curl http://www.modcoderpack.com/files/mcp940.zip --out MCP-1.12.0.zip
cls
echo Unzipping MCP-1.12.0...
powershell -Command "Expand-Archive MCP-1.12.0.zip"
del MCP-1.12.0.zip
cls
echo Setting up MCP-1.12.0... (this will take a long time)
cd MCP-1.12.0
runtime\bin\python\python_mcp runtime\decompile.py %*
cls
goto end
